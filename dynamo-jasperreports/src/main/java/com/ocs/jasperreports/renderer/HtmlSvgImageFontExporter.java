...
/**
 * This class is used to override the part where the HtmlExporter fails to use the fonts inside the SVG in cases where it's rendered as an image
 * Exports to SVG xml are already compatible with (custom) fonts
 * Changed parts are surrounded with // CUSTOMIZED and // END CUSTOMIZED
 */
public class HtmlSvgImageFontExporter extends HtmlExporter {
	...
	protected void writeImage(JRPrintImage image, TableCell cell) throws IOException, JRException {
		...
		int availableImageWidth =
				image.getWidth() - image.getLineBox().getLeftPadding() - image.getLineBox().getRightPadding();
		if (availableImageWidth < 0) {
			availableImageWidth = 0;
		}

		int availableImageHeight =
				image.getHeight() - image.getLineBox().getTopPadding() - image.getLineBox().getBottomPadding();
		...
		if (renderer != null) {
			...
			if (scaleImage == ScaleImageEnum.CLIP || (isLazy && (
			...
			boolean hasAreaHyperlinks =
					renderer instanceof AreaHyperlinksRenderable && ((AreaHyperlinksRenderable) renderer)
			...
			if (hasAreaHyperlinks) {
				...
				if (imageMapName == null) {
					...
					imageMapName = "map_" + getElementIndex(cell).toString() + "-" + originalRenderer
							.getId();//use renderer.getId()?
			...
			if (imageProcessorResult != null) {
				if (useBackgroundLazyImage) {
					...
					writer.write(
							"'); background-repeat: no-repeat; background-position: " + horizontalAlignment + " " + (
									image.getVerticalImageAlign() == VerticalImageAlignEnum.MIDDLE ?
											"center" :
											verticalAlignment) + ";background-size: ");
					...
					case CLIP: {
						...
						if (isLazy || (horizontalAlign == HorizontalImageAlignEnum.LEFT
							...
							positionLeft = (int) (ImageUtil.getXAlignFactor(horizontalAlign) * (availableImageWidth
									- normalWidth));
							positionTop = (int) (ImageUtil.getYAlignFactor(verticalAlign) * (availableImageHeight
									- normalHeight));
						}
		...
		protected InternalImageProcessorResult process(Renderable renderer) throws JRException, IOException {
			...
			if (isLazy) {
				...
				if (!embedImage //we do not cache imagePath for embedded images because it is too big
						...
						//we do not cache imagePath for non-data renderers because they render width different width/height each time
					...
					if (embedImage) {
						...
						if (isConvertSvgToImage(imageElement)) {
							dataRenderer = getRendererUtil().getImageDataRenderable(renderersCache, renderer,
									...
									ModeEnum.OPAQUE == imageElement.getModeValue() ?
											imageElement.getBackcolor() :
											null);
						} else {
						...
						if (svgInfo != null) {
							...
							DimensionRenderable dimensionRenderer = renderersCache.getDimensionRenderable(renderer);
							dimension = dimensionRenderer == null ?
									null :
							...
							imageSource = "data:" + imageMimeType + ";base64," + new String(baos.toByteArray(),
									"UTF-8"); // UTF-8 is fine as we just need an ASCII compatible encoding for the Base64 array
						}
						...
						HtmlResourceHandler imageHandler =
								getImageHandler() == null ? getExporterOutput().getImageHandler() : getImageHandler();
