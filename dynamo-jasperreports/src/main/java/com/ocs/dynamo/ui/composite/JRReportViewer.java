...
/**
 * Custom component to render HTML versions of JasperReports and export to other
 * formats
 * 
 * @author Patrick Deenen (patrick@opencircle.solutions)
 *
 */
public class JRReportViewer<ID extends Serializable, T extends AbstractEntity<ID>> extends BaseCustomComponent {
	...
	/**
	 * Indicates whether external script has already been loaded (currently only
	 * works for one external script!)
	 */
	...
	private AbstractSingleSelect<?> reportSelection;
	...
	protected Component buildReportSelection() {
		...
		reportSelection.setRequiredIndicatorVisible(true);
	...
	public void displayReport(SerializablePredicate<T> filter, Map<String, Object> parameters, Locale locale) {
		// TODO make the report generation asynchronous and display the first page when
		// it is ready
		// and not after last
		// page is ready
		...
		if (!rd.requiresDatabaseConnection()) {
			jrDataSource = new JRIndexedContainerDataSource<ID, T>(provider);
		}
		...
		if (html == null || "".equals(html) || (provider != null)) {
	...
	public AbstractSingleSelect<?> getReportSelection() {
	...
	public void setProvider(BaseDataProvider<ID, T> provider, boolean defineProperties) {
		...
		if (defineProperties) {
			...
			for (ReportDefinition type : (ReportDefinition[]) reportDefinition.getClass().getEnumConstants()) {
				if (!type.requiresDatabaseConnection()) {
					JRUtils.addContainerPropertiesFromReport(provider, reportGenerator.loadTemplate(getFullPath(type)));
				}
			}
		}
	}
