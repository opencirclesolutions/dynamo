...
public class DomainUtilsTest extends BaseMockitoTest {
    ...
    public void testCreateIfNotExists_Create() {
        ...
        Country c = captor.getValue();
        Assert.assertNull(c.getCode());
        Assert.assertEquals("SomeName", c.getName());
    }

    @Test
    public void testCreateIfNotExists_CreateWithCode() {
        MockUtil.mockServiceSave(service, Country.class);

        Country country = DomainUtil.createIfNotExists(service, Country.class, "SomeCode", "SomeName", true);
        Assert.assertNotNull(country);
        ...
        Country c = captor.getValue();
        Assert.assertEquals("SomeCode", c.getCode());
        Assert.assertEquals("SomeName", c.getName());
    }
    ...
    @Test
    public void testCreateIfNotExistsWithCode_DoNotCreate() {
        Country existing = new Country();
        existing.setCode("SomeCode");
        existing.setName("SomeName");
        existing.setId(44);
        Mockito.when(service.findByUniqueProperty(Domain.CODE, "SomeCode", true)).thenReturn(existing);
