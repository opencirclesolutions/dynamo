...
public class DomainUtilsTest extends BaseMockitoTest {
	...
	public void testGetDomainDescriptions() {
		...
		Mockito.when(
				messageService.getMessage(Mockito.eq("ocs.and.others"), Mockito.any(Locale.class), Mockito.anyInt()))
	...
	public void testCreateIfNotExists_Create() {
		...
		Mockito.verify(service).save(Mockito.any(Country.class));
	}
	...
	public void testCreateIfNotExists_DoNotCreate() {
		...
		// verify that a country is saved
		Mockito.verify(service, Mockito.times(0)).save(Mockito.any(Country.class));
	}
