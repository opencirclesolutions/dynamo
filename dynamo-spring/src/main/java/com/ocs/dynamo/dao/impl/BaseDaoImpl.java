 ...
 */
public abstract class BaseDaoImpl<ID, T extends AbstractEntity<ID>> implements
		BaseDao<ID, T> {
	...
	private void addSorting(JPAQuery query, SortDefinition... sorts) {
		...
		for (SortDefinition s : sorts) {
			...
			query.orderBy(new OrderSpecifier(s.isAscending() ? Order.ASC
					: Order.DESC, property));
		}
	}
	...
	public long count(Filter filter, boolean distinct) {
		CriteriaQuery<Long> cq = JpaQueryBuilder.createCountQuery(
				entityManager, getEntityClass(), filter, distinct);
	...
	public T fetchById(ID id, FetchJoinInformation... joins) {
		CriteriaQuery<T> cq = JpaQueryBuilder.createFetchSingleObjectQuery(
				entityManager, getEntityClass(), id,
	...
	@Override
	public List<T> fetchByIds(List<ID> ids, Sort sort,
			FetchJoinInformation... joins) {
		CriteriaQuery<T> cq = JpaQueryBuilder.createFetchQuery(entityManager,
				getEntityClass(), ids, sort,
				(joins != null && joins.length > 0) ? joins : getFetchJoins());
		TypedQuery<T> query = entityManager.createQuery(cq);
	...
	@Override
	public List<T> find(Filter filter, Sort sort,
			FetchJoinInformation... fetchJoins) {
		CriteriaQuery<T> cq = JpaQueryBuilder.createSelectQuery(filter,
				entityManager, getEntityClass(), fetchJoins, sort);
	...
	@Override
	public List<T> find(Filter filter, Pageable pageable,
			FetchJoinInformation... fetchJoins) {
		// Create select and where clauses
		CriteriaQuery<T> cq = JpaQueryBuilder.createSelectQuery(filter,
				entityManager, getEntityClass(), fetchJoins,
	...
	public T findByUniqueProperty(String propertyName, Object value,
		...
		CriteriaQuery<T> cq = JpaQueryBuilder.createUniquePropertyQuery(
				entityManager, getEntityClass(), propertyName, value,
				caseSensitive);
		TypedQuery<T> query = entityManager.createQuery(cq);
		try {
			return query.getSingleResult();
			...
			throw new OCSRuntimeException(
					"Query for unique property returned multiple results", ex);
		}
	}

	@Override
	public T fetchByUniqueProperty(String propertyName, Object value,
			boolean caseSensitive, FetchJoinInformation... joins) {
		CriteriaQuery<T> cq = JpaQueryBuilder.createUniquePropertyFetchQuery(
	...
	public List<ID> findIds(Filter filter, Sort sort) {
		CriteriaQuery<Tuple> cq = JpaQueryBuilder.createIdQuery(entityManager,
				getEntityClass(), filter, sort);
