    ...
    private void addColumn(final AttributeModel attributeModel, List<Object> propertyNames, List<String> headerNames) {
        if (attributeModel.isVisibleInTable()) {
            ...
            if (container instanceof LazyQueryContainer) {
                ...
                BeanItemContainer<?> beanContainer = (BeanItemContainer<?>) container;
                if (!beanContainer.getContainerPropertyIds().contains(attributeModel.getPath())) {
                    beanContainer.addNestedContainerProperty(attributeModel.getPath());
                }
            }
