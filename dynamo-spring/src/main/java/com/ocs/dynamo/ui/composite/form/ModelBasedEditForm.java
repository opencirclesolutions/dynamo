...
public class ModelBasedEditForm<ID extends Serializable, T extends AbstractEntity<ID>>
    ...
    /**
     * Map that holds attribute groups per view mode
     */
    ...
    public ModelBasedEditForm(T entity, BaseService<ID, T> service, EntityModel<T> entityModel, FormOptions formOptions,
        ...
        buttons.put(Boolean.TRUE, new ArrayList<Button>());
        buttons.put(Boolean.FALSE, new ArrayList<Button>());
    }
    ...
    /**
     * Adds a tab change listener that gives focus to the first field on a tab after the tab is
     * selected
     * 
     * @param tabSheet
     */
    ...
    /**
     * Constructs the main layout of the screen - this layout contains buttons bars and an edit form
     * 
     * @param entityModel
     *            the entity model used to construct the layout
     * @return
     */
        ...
        saveButton.addClickListener(new Button.ClickListener() {
            ...
            public void buttonClick(ClickEvent event) {
                try {
                    ...
                    setEntity(service.fetchById(entity.getId(), getDetailJoins()), false);
    ...
    private Label constructTitleLabel() {
        ...
        if (SystemPropertyUtils.getDefaultFormTitleWidth() > 0) {
    ...
    private List<Button> filterButtons(String data) {
        ...
        for (Button b : buttons.get(isViewMode())) {
