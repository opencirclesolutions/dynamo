...
public abstract class AbstractModelBasedSearchForm<ID extends Serializable, T extends AbstractEntity<ID>>
    ...
    public void build() {
        if (main == null) {
            ...
            if (filterLayout.isVisible()) {
                ...
                // button bar is empty by default, but allow the user to add custom buttons
                buttonBar = new DefaultHorizontalLayout();
                main.addComponent(buttonBar);
                postProcessButtonBar(buttonBar);
            }
    ...
    /**
     * Method that is called before a search is carried out - use this to carry out additional
     * validations, e.g. if certain combinations of search fields are filled
     */
