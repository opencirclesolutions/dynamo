...
change the value of the *searchForExactValue* setting to _true_; if
you do this then only a single search field will appear.
*** For a property of type *LocalDateTime* or *ZonedDateTime* you can set
the *searchForDate* setting to _true_. If you do this then only a single
...
*complexEditable* setting of the attribute model to _true_. Note that in
...
[source,java]
--
@Attribute(visibleInGrid = VisibilityType.SHOW, searchable = SearchMode.ALWAYS, complexEditable = true, cascade =
...
This setting defaults to _false_.

When set to _true_, for an attribute of type *DETAIL*, by default a
...
setting is changed to _true_ for a property, then a currency symbol will
...
system property *ocs.capitalize.words* and set it to _false_ so that
...
“*readOnly*” and could only be used to toggle between _editable_ and
_read-only state_.
...
valid email address. It is automatically set to _true_ if the property
...
By setting *embedded* to _true_, we effectively make sure that there is no
...
represents an image. By default, this setting has the value _false_.
If set to _true_, the application will try to render a preview image of
...
property *ocs.use.grid.selection.checkboxes* (if set to _true_, then the
...
attributes, but if you set this setting to _true_ you will be allowed to
...
generate a text field. However, if you set *multipleSearch* to _true_ and
...
_false_. If set to _true_, then the value of the property will be
...
We set the *quickAddAllowed* to “_true_”. Once the user now starts the
...
*requiredForSearching* set tot _true_, you will not be able to carry out a
...
property *ocs.default.search.case.sensitive* which defaults to “_false_”.
...
search interval. When you change this setting to _true_ then instead the
...
By default, this setting has the value _false_. If set to _true_, then
...
the property check only for a prefix match. If this is set to _true_,
then searching for e.g. “a” will only match “almond” (“a” appears at
start) but not “walnut” (“a” appears in the middle). If set to _false_,
then “a” will match both “almond” and “walnut”.

By default, this setting has the value _false_. This setting is only
...
will be shown. When this setting has the value _false_ (the default),
then the contents of the password field will always be shown obscured.
When this setting has the value _true_, a button to toggle between an
...
sorted on the attribute. By default, it is set to _true_ for all
attributes.
...
supports the values *INHERIT, TRIM* and *NO_TRIM*. When _INHERIT_ is used, it
will just use the value of the system property. With _TRIM_ and _NO_TRIM_
...
default, such a value will simply by displayed as “_true_” or “_false_”, but
...
The default value is _false_. If set to _true_, then a validator will
...
inside an edit form. It is not to be confused with the
*visibleInGrid* attribute that governs whether a property shows up in a
grid.

By default, all properties will have *visible* set to _true_, except for
...
*visibleInGrid* setting to _true_.

Instead of _true_ you can also use the value *SHOW* and instead of
_false_ you can also sue the value *HIDE*.
...
_true_, except for the “*id*” property which is used for a technical primary
...
_false_. If set to _true_, then instead of a date picker, a text field
...
--
In its most basic form, you can define a service by extending the
...
ApplicationConfigurationSupport {
    ...
    protected String[] getBaseNames() {
        return new String[] { "classpath:/META-INF/entitymodel",
                    "classpath:/menu", "classpath:/ui.messages",
...
public class GtsSecurityAdapter extends WebSecurityConfigurerAdapter {
    ...
    public void configureGlobal(AuthenticationManagerBuilder auth) throws Exception {
        auth.inMemoryAuthentication().withUser("Dynamo")
                .password("{noop}Dynamo")
                .authorities("user");
    }
...
public class GtsUI extends VerticalLayout implements RouterLayout {
    ...
    private static final long _serialVersionUID_ = -4652393330832382449L;
...
public class OrganizationView extends BaseView {
    ...
    private static final long _serialVersionUID_ = 3310122000037867336L;
...
pass “_true_” to the constructor parameter *confirmBeforeLeave.*
...
FormOptions fo = new FormOptions().setAttributeGroupMode(AttributeGroupMode._TABSHEET_).setShowIterationButtons(true);

SimpleSearchLayout<Integer, Organization> layout = new SimpleSearchLayout<>(organizationService, em, QueryType._ID_BASED_,
fo,
...
* *complexDetailsMode –* When this is set to _true_, instead of displaying
...
* *confirmClear –* If this is set to _true_ the user will be asked for
confirmation if the user presses the *Clear* button to clear all search
filters in a search form.
* *confirmSave* – If this is set to _true_ the user will be asked for
...
appear on the same row as the edit components. If set to _false_ the
...
different from setting *showEditButton* to _false_ – setting *editAllowed*
to false ensures there is still a detail view but it cannot be used in
edit mode. Setting *detailsModeEnabled* to _false_ completely turns off
the detail view.
...
*openInViewMode* to true and this setting to _false_ if you want to
disable editing for a component.
* *enableAdvancedSearchMode* – by setting this to _true_, you enable the
...
* *placeButtonBarAtTop* – If set to _true_, the application will display
...
search screen and then back. It defaults to _true_.
...
hides the appropriate buttons and sets *openInViewMode* to _true_).
...
[source,java]
--
layout.setDetailsModeTabCaptions(new String[] { "Organizations", "Members" });
--
...
layout.addDetailTabCreator(0, (formOptions, isNew) -> {
    ...
    return new SimpleEditLayout<Integer, Organization>(layout.getSelectedItem(), organizationService, em2, formOptions, new FetchJoinInformation("members"), new FetchJoinInformation("countryOfOrigin"));
});
--
...
*setDetailsModeTabTitle*() method to set it.
...
ServiceBasedSplitLayout<Integer, Person> layout = new ServiceBasedSplitLayout<Integer, Person>(personService, em,
...
form). The default value is *HORIZONTAL*.

For the *ServiceBasedSplitLayout* you can also set the
*showQuickSearchField* property to _true_ or _false_ to specify whether a
quick search field must appear above the results grid. The default value
is _false_. If you set it to _true_, then the screen will contain a
...
[source,java]
--
public EditableGridLayout(BaseService<ID, T> service, EntityModel<T>entityModel, FormOptions formOptions, SortOrder sortOrder, FetchJoinInformation...joins) {
...
grid mode: if you set *openInViewMode* to _true_, the grid will
...
tabLayout.setTabCreator(index -> {
    SerializablePredicate<Programme> filter = fsf.extractFilter() == null ? statusFilter : new AndPredicate<>(fsf.extractFilter(), statusFilter);
    switch (index) {
        case _INDEX_PROGRAM_LIST_:
            return constructProgrammeListLayout(() -> fsf.extractFilter() == null ? statusFilter : new AndPredicate<>(fsf.extractFilter(), statusFilter), tabLayout.getEntity());
        case _INDEX_PROGRAM_COUNT_:
...
can use a *DetailsEditGrid*. This is a component that you must
...
*detailsGridSearchMode* on the FormOptions parameter object to _true_:
...
[source,java]
--
ServiceBasedDetailsEditGrid<Integer, Person, Integer, Organization> grid = new ServiceBasedDetailsEditGrid<>(personService, getModelFactory().getModel(Person.class), attributeModel, viewMode, new FormOptions().setDetailsGridSearchMode(false).setShowRemoveButton(true));
...
[source,java]
--
DetailsEditLayout<Integer, GiftTranslation> dt = new DetailsEditLayout<Integer, GiftTranslation>(giftTranslationService, getEntityModelFactory().getModel(GiftTranslation.class), attributeModel, viewMode, fo, new AttributeComparator<>("description"));
...
popupButton.addClickListener(event -> {
    EntityPopupDialog<Integer, Organization> popupDialog = new EntityPopupDialog<Integer, Organization>(organizationService, getSelectedItem(), getEntityModelFactory().getModel(Organization.class), new HashMap<>(), new FormOptions());
...
using the *FormOptions* and setting *openInViewMode* to _true_.
...
form is only created the first time the user presses the Add or
...
[source,java]
--
setParentGroupHeaders(new String[] { "blip.attributes.main", _MSG_ATTRIBUTES_PROMOTED_, "blip.attributes.other" });
...
have to set the system property *ocs.allow.list.export* to _true_. This
...
can use the *setExportAllowed* setting on the FormOptions to _true_.
...
image:media/image15.png[image,width=186,height=189]
...
public BaseDao<Integer, Country> countryDao() {
    return new DefaultDaoImpl<>(QCountry._country_, Country.class, "parent");
}
...
<entity-mappings xmlns="http://java.sun.com/xml/ns/persistence/orm" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xsi:schemaLocation="http://java.sun.com/xml/ns/persistence/orm http://java.sun.com/xml/ns/persistence/orm_1_0.xsd" version="1.0">
    ...
    <sequence-generator name="domain_id_seq" allocation-size="1" sequence-name="domain_id_seq"/>
    <sequence-generator name="parameter_id_seq" allocation-size="1" sequence-name="parameter_id_seq">
...
image:media/image16.png[image,width=559,height=226]
...
static *FieldFactory.getInstance*() method.
...
pace an *@CssLayout* on the class in your application that implements
RouterLayout:
...
[source,css]
--
border: 1px solid_var(lumo-primary-color) !important;
--
...
*spring.jpa.hibernate.ddl-auto* property to “_auto_”. This will cause
...
public class VersionedOrganization extends VersionedEntity<Integer, Organization> {
    ...
    private static final long _serialVersionUID_ = 4271855110308393329L;
    ...
    public String getDescription() {
        return RevisionType._DEL_.equals(getRevisionType()) ? "Deleted" : getEntity().getName();
    }
}
--
...
--
@Repository
public class VersionedOrganizationDaoImpl extends VersionedEntityDaoImpl<Integer, Organization, VersionedOrganization> implements VersionedOrganizationDao {
...
@Service
public class VersionedOrganizationServiceImpl extends BaseServiceImpl<RevisionKey<Integer>, VersionedOrganization> implements VersionedOrganizationService {
...
SimpleSearchLayout<RevisionKey<Integer>, VersionedOrganization> versLayout = new SimpleSearchLayout<>(
        versionService, getModelFactory().getModel(VersionedOrganization.class), QueryType.ID_BASED, new FormOptions().setReadOnly(true), null);
...
[source,java]
--
ViewRevisionDialog<Integer, Organization, VersionedOrganization> dialog = new ViewRevisionDialog(versionService, getModelFactory().getModel(VersionedOrganization.class), new FormOptions().setAttributeGroupMode(AttributeGroupMode.TABSHEET), layout.getSelectedItem().getId());
...
image:media/image17.png[image,width=560,height=629]
