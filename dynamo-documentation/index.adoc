...
integers and Strings (note that the JPA spec does allow more complex element
...
create the services and DTOs that are required by the Dynamo framework.

It should (at least in principle) be possible to add Dynamo to your existing front-end application
without too much effort. Note that setting up the general structure of your application is largely
outside of the scope of the Dynamo Framework (at least for now). Notably, Dynamo does not concern itself
with the application's menu structure or routing, although it does offer some convenience features and
general use services.


[TODO] include a part about using the Dynamo component
...
As an alternative, https://curl.se[curl] can be used to download this file:
...
Save the data to a file named "api-docs.yaml" and save it in the root directory of your
front-end application. Then, run the following command:

[source,commandline]
--
npx @openapitools/openapi-generator-cli generate -i api-docs.yaml -g typescript-angular -o ./model/ --additional-properties=enumPropertyNaming=original,ngVersion=16
--
...
As an alternative, https://curl.se[curl] can also be used to download this
file:
