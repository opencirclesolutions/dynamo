...
system property “ocs.default.boolean.field.mode”. It defaults to
...
the system properties *ocs.default.date.format* _(dd-MM-yyyy),_
*ocs.default.time.format* _(HH:mm:ss),_ *ocs.default.datetime.format*
_(dd-MM-yyyy HH:mm:ss)_.
...
framework will default to the value of the *ocs.default.date.format,
ocs.default.time.format,* *ocs.default.datetime.format,* or system
...
*ocs.default.enum.field.mode.* When the value is changed to *RADIO* a
...
=== GroupTogetherWith (not supported yet)
...
@ComponentScan(basePackages = {"com.opencircle.gts","com.ocs.dynamo"})
@EntityScan({"com.ocs.dynamo.functional.domain", "com.opencircle.gts.domain"})
...
|ocs.default.boolean.field.mode |CHECKBOX |The default compoment to use
...
|ocs.default.element.collection.mode |CHIPS |The default component to
use for managing an element collection

|ocs.default.enum.field.mode |DROPDOWN |The default component to use for
...
|ocs.default.boolean.field.mode |CHECKBOX |The default component to use
...
|ocs.entity.model.package.names | |A comma-separated list of packages
...
[source,commandline]
--
npx @openapitools/openapi-generator-cli generate -i api.yaml -g
typescript-angular -o ./model/
--additional-properties=enumPropertyNaming=original,ngVersion=16
--
...
export function createTimestampFilter(attributeName: string, from: Date,
to: Date, instant: boolean) \{
...
The *OrFilter* can be used to construct a filter that will match if at
least of a provided collection of filters matches.

*NotFilter*

    return
[createNotFilter(createEqualsFilter('reservationCategory','RESERVED_EXTRA'))
...
(afterEntityCreated)="afterEntityCreated(_$event_)" +
> +
<ng-template +
dHiddenField +
...
@Override +
@ModelAction(id = "AddMinimalOrganization", displayName = "Quick Add
organization", +
icon = "pi-building") +
@Transactional +
...
org.setFounded(LocalDate._now_()); +
org.setCountryOfOrigin(dto.getCountryOfOrigin()); +
org.setReputation(Reputation._FEARSOME_); +
...
@Roles(readRoles = \{}, writeRoles = \{"hidden"}, deleteRoles =
\{"hidden"})
...
__ +
__void validateReadAllowed(EntityModel<?> model); +
+
...
\{"id", "name", "address", "headQuarters", "countryOfOrigin"}) +
@AttributeGroup(messageKey = "organization.second", attributeNames =
\{"memberCount", "yearlyMortalityRate", "mainActivity", +
...
 attributeVisible(am: AttributeModelResponse, editObject: any,
formGroup: FormGroup) \{
...
*ocs.openai.enabled* application property to “true”. In addition, values
...
With this in place, you need to provide the application with your
...
parameter *ocs.ai.default.service* This setting accepts the values
...
In the Typescript definition, you can used **@ViewChi**ld to inject a
...
The *GenericSplitLayout* is a composite layout component that contains a
