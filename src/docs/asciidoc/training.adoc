...
and to set a `displayProperty` for the Organization if that has not been done already).
...
[source,html]
--
<app-generic-edit-table-layout entityName="Delivery"></app-generic-edit-table-layout>
--
...
getFieldFilters() {
  ...
  const countryFilter = createEqualsFilter('name', 'au');
...
@Column(name = "lucky_number")
@Size(max = 3)
@Attribute(visibleInForm = VisibilityType.SHOW, minValue = 0, maxValue
= 100)
