...
public abstract class ProgressForm<T> extends BaseCustomComponent implements Progressable {
	 ...
	 * 
	 * @param progressMode
	 *            indicates the mode of the screen (and whether the progress bar
	 *            will be displayed)
	 */
	 ...
	 * 
	 * @param main
	 *            the layout that serves as the container for everything that is
	 *            being added
	 */
	protected abstract void doBuildLayout(Layout main);

	/**
	 * Estimates the current progress - overwrite this method if you want to use
	 * a custom mechanism for updating the progress
	 * 
	 * @return
	 */
	@Override
	...
	/**
	 * Renders the "form" mode which allows the user to input data before
	 * starting the
	 */
	...
	/**
	 * Indicates whether the form is valid for processing. By default this
	 * method returns <code>true</code>. Override if needed.
	 * 
	 * @param t
	...
	/**
	 * Prepares the screen for the actual processing - this is the method that
	 * must be called from an action listener in order to start the process
	 * 
	 * @param t
	 *            the (optional) object that is being processed
	 * @param fileName
