...
public class PivotGridWrapper<ID extends Serializable, T extends AbstractEntity<ID>> extends GridWrapper<ID, T, PivotedItem> {
    ...
    /**
     * Bi-function used to map pivot column headers
     */
     ...
     */
    public PivotGridWrapper(BaseService<ID, T> service, EntityModel<T> entityModel, QueryType queryType, FormOptions formOptions,
            SerializablePredicate<T> filter, List<SortOrder<?>> sortOrders, FetchJoinInformation... joins) {
    ...
    protected PivotDataProvider<ID, T> constructDataProvider() {
        ...
        PivotDataProvider<ID, T> pivotDataProvider = new PivotDataProvider<>(wrappedProvider, rowKeyProperty, columnKeyProperty,
                fixedColumnKeys, pivotedProperties, sizeSupplier);
