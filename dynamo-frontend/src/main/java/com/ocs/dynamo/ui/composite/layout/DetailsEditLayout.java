...
/**
 * A layout for displaying various nested forms below each other. This is intended
 * for use inside a ModelBasedEditFOrm
 *
...
public class DetailsEditLayout<ID extends Serializable, T extends AbstractEntity<ID>, ID2 extends Serializable, Q extends AbstractEntity<ID2>>
    ...
    class FormContainer extends DefaultVerticalLayout implements Buildable {
        ...
        private final ModelBasedEditForm<ID, T> form;

        private final int index;

        /**
         * Consumer that is used to post-process the detail button bar. Takes as its parameters
         * the zero-based index of the detail form, the button bar, and a boolean indicating
         ...
         * Constructor
         *
         * @param index the index of the form (zero based)
        ...
        private void addRemoveButton() {
            if (!viewMode && getFormOptions().isShowRemoveButton()) {
                ...
                removeButton.addClickListener(event -> {
                    ...
                    removeEntity.accept(enclosingForm == null ? null : enclosingForm.getEntity(), this.form.getEntity());
        ...
        public void build() {
            if (buttonBar == null) {
                ...
                if (getFormOptions().isDetailsEditLayoutButtonsOnSameRow()) {
                    // add form and button bar on the same row, one after the other
                    HorizontalLayout rowLayout = new DefaultHorizontalLayout(false, false);
                    rowLayout.addClassName(DynamoConstants.CSS_DETAILS_EDIT_LAYOUT_ROW);
                    ...
                    // add form followed by button bar
                    add(form);
                    add(buttonBar);
    ...
    /**
     * The button that can be used to add additional forms
     */
    @Getter
    ...
    /**
     * The comparator (will be used to sort the items that are displayed in the sub-forms)
     */
    @Getter
    @Setter
    ...
    private final List<FormContainer> forms = new ArrayList<>();
    ...
    /**
     * The container that holds all the sub-forms
     */
     ...
     *
     * @param path      the path to the attribute for which to add a custom converter
     * @param converter the supplier used to construct the converter
     */
     ...
     * Adds a custom field
     *
     * @param path     the path to the attribute for which to add a custom field
     * @param function the function used to construct the component
     */
	 ...
	 * @param path     the path to the attribute for which to add a custom validator
	 * @param validator the supplier used to construct the validator
	 */
    ...
    private void addDetailEditForm(int index, T entity) {
        ...
        ModelBasedEditForm<ID, T> editForm = new ModelBasedEditForm<>(entity, service, entityModel, formOptions,
                fieldFilters);
    ...
    protected final void constructAddButton(FlexLayout buttonBar) {
        ...
        addButton.addClickListener(event -> {
            T entity = createEntity.apply(getEnclosingForm() == null ? null : getEnclosingForm().getEntity());
            items.add(entity);
            addDetailEditForm(getFormCount(), entity);
        });
    ...
    /**
     * Constructs the button bar that is displayed below all the forms
     *
