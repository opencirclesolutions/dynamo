...
public class PivotSearchLayout<ID extends Serializable, T extends AbstractEntity<ID>> extends AbstractSearchLayout<ID, T, PivotedItem> {
     ...
     * @param joins
     */
    public PivotSearchLayout(BaseService<ID, T> service, EntityModel<T> entityModel, FormOptions formOptions, SortOrder<?> sortOrder,
    ...
    public PivotGridWrapper<ID, T> constructGridWrapper() {
        ...
        PivotGridWrapper<ID, T> wrapper = new PivotGridWrapper<ID, T>(getService(), getEntityModel(), QueryType.ID_BASED, getFormOptions(),
                getSearchForm().extractFilter(), getSortOrders(), getJoins()) {
