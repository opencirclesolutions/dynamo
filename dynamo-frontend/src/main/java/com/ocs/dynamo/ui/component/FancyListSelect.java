 ...
 */
public class FancyListSelect<ID extends Serializable, T extends AbstractEntity<ID>> extends QuickAddEntityField<ID, T, Collection<T>>
    ...
    public FancyListSelect(BaseService<ID, T> service, EntityModel<T> entityModel, AttributeModel attributeModel,
        ...
        comboBox = new EntityComboBox<>(getEntityModel(), getAttributeModel(), getService(), getFilter(), null, sortOrders);
    }

    /**
     * Adds a value changes listener. This delegates to the underlying list select
     * component
     */
    @Override
    public Registration addValueChangeListener(ValueChangeListener<Collection<T>> listener) {
        if (listSelect != null) {
            ValueChangeListener<Set<T>> list = event -> listener.valueChange(new ValueChangeEvent<Collection<T>>(FancyListSelect.this,
                    FancyListSelect.this, (Collection<T>) listSelect.getValue(), true));
    ...
    /**
     * Method that is called after a new entity was added using the quick add
     * functionality. Adds the entity to the combo box and list select
     */
    @Override
