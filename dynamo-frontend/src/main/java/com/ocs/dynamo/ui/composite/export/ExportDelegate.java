...
public interface ExportDelegate {
	 ...
	 */
	<ID extends Serializable, T extends AbstractEntity<ID>> void exportFixed(EntityModel<T> entityModel,
			ExportMode mode, Collection<T> items);

	/**
	 * Exports a pivoted data set
	 * 
	 * @param <ID>            the type of the primary key of the pivoted item
	 * @param <T>             the type of the pivoted item
	 * @param entityModel     the entity model
	 * @param predicate       the filter predicate
	 * @param sortOrders      the sort orders
	 * @param pivotParameters the parameters that determine how to do the pivoting
