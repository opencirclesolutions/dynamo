...
/**
 * This container pivots data using another container that actually provides the data.
 * 
...
public class PivotContainer implements Container, ItemSetChangeListener, Indexed, Sortable {
     ...
     *            Mandatory, must implement container.Indexed. source container supplies the source
     *            data to pivot
     * @param columnPropertyId
     *            Mandatory defines the property which contains the column designator
     * @param rowPropertyId
     *            Mandatory defines the property which contains the row designator
     * @param columnIds
     *            Mandatory, size should be at least 1 defines all possible unique column values
     * @param pivotedColumnPostFixes
     *            The identifiers that should be placed behind each column ID in order to generate
     *            the columns (this is used to prevent the creation of excessive columns)
     * @param rowCount
