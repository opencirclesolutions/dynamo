...
public abstract class BaseExcelExportTemplate<ID extends Serializable, T extends AbstractEntity<ID>> extends BaseExportTemplate<ID, T> {
    ...
    public BaseExcelExportTemplate(BaseService<ID, T> service, EntityModel<T> entityModel, ExportMode exportMode, SortOrder[] sortOrders,
    ...
    protected void writeCellValue(Cell cell, Object value, EntityModel<T> em, AttributeModel am) {
        if (NumberUtils.isInteger(value) || NumberUtils.isLong(value)) {
            ...
            if (isPercentage) {
                ...
                double temp = ((BigDecimal) value).divide(MathUtils.HUNDRED, DynamoConstants.INTERMEDIATE_PRECISION, RoundingMode.HALF_UP)
                        .setScale(am.getPrecision() + defaultPrecision, RoundingMode.HALF_UP).doubleValue();
                cell.setCellValue(temp);
            } else {
                cell.setCellValue(((BigDecimal) value).setScale(am == null ? defaultPrecision : am.getPrecision(), RoundingMode.HALF_UP)
                        .doubleValue());
            }
        } else if (am != null) {
